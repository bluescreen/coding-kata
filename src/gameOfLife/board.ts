...
export default class Board {
  ...
  setState(state: string): void {
    ...
    for (let i = 0; i < states.length; i++) {
      this.cells[i].state =
        states[i] === '.' ? CellState.DEAD : CellState.ALIVE;
    }
  }
  ...
  findLivingNeighbours(index) {
    return this.findNeighbours(index).filter(
      (cell: Cell) => cell.state === CellState.ALIVE,
    ).length;
  }
  ...
  update() {
    ...
    for (let i = 1; i < updated.length; i++) {
      const isAlive = this.cells[i].state;
