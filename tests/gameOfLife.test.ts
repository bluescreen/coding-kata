...
describe('Convays game of life', () => {
  ...
  describe('Find coords on board', () => {
    it('should find coords for index', () => {
      const board = new Board(2, 4);
      expect(board.findCoordsFor(1)).toEqual({ row: 1, col: 1 });
      expect(board.findCoordsFor(2)).toEqual({ row: 1, col: 2 });
      expect(board.findCoordsFor(3)).toEqual({ row: 1, col: 3 });
      expect(board.findCoordsFor(4)).toEqual({ row: 1, col: 4 });
      expect(board.findCoordsFor(5)).toEqual({ row: 2, col: 1 });
      expect(board.findCoordsFor(6)).toEqual({ row: 2, col: 2 });
      expect(board.findCoordsFor(7)).toEqual({ row: 2, col: 3 });
      expect(board.findCoordsFor(8)).toEqual({ row: 2, col: 4 });
    });
  });
